...
void parse_msg(struct client_ctx* client) {
    ...
    if (!strncmp(cl.in_buff+cl.new_msg, "add meter", 9)) {

    }
    if (cl.in_buff[cl.new_msg-1] == 'a') {
        //add_new_value();//TODO: NOT IMPLEMENTED
        cout << "added" << endl;
    }
    else if (cl.in_buff[cl.new_msg-1] == 'h') {
        //send_history();//TODO: NOT IMPLEMENTED
        cout << "history sent" << endl;
    }
    else if (cl.is_admin == 2 && cl.in_buff[cl.new_msg-1] == 'g') {
        //send_debtors();//TODO: NOT IMPLEMENTED
        cout << "debtors sent" << endl;
    }
    else {
        char msg[512];
        sprintf(msg, "Команда не распознана\n");
        send_msg(client, msg);
    }
        ...
        for (int i = 0; i<events_cnt; i++) {
            ...
            if (e->data.fd >= 0) {
                ...
                if ((e->events & EPOLLHUP) || (e->events & EPOLLRDHUP) || (e->events & EPOLLERR)) {
                    ...
                    ssize_t r = recv(g_ctxs[idx].socket, g_ctxs[idx].in_buff + g_ctxs[idx].received, sizeof(g_ctxs[idx].in_buff - g_ctxs[idx].received), 0);
                    if (r>0) {
